...
- [Python 3.14](https://python.org) — Linguagem de programação foco do estudo.
- [Google NotebookLM](https://notebooklm.google) — Ferramenta de IA utilizada para sintetizar as fontes e gerar os roteiros didáticos.
- **Ambientes Virtuais (`venv`):** Ferramentas para isolamento de pacotes do ecossistema Python.

## 📁 Estrutura de Como Praticar este Roteiro

Para exercitar os conceitos propostos pelo caderno na sua máquina local, recomenda-se criar um arquivo inicial de testes:

```bash
# 1. Prepare seu ambiente e crie uma pasta
mkdir meu-projeto-python && cd meu-projeto-python

# 2. Crie o arquivo principal de estudos
touch app.py
```

Dentro de `app.py`, você pode colar o exemplo básico de tomada de decisão estruturado no caderno:

```python
# Exemplo básico de decisão (if/else) do guia
nota = 8
...
## 📚 Recursos Recomendados nas Fontes
O material destaca excelentes referências externas para prática:
- *Automate the Boring Stuff with Python* (Ideal para automação de rotinas).
- *Think Python* e *How To Think Like a Computer Scientist*.
- Plataformas interativas: Codecademy, DataCamp e CheckiO.
